 ...
 * peripheral half.
 *
 * Step 1 of an incremental rebuild after the full version (this info
 * panel + an animated cat widget) caused a boot crash on hardware.
 * This step is text-only: plain lv_label_create calls, same pattern
 * ZMK's own built-in widgets use, no custom shapes/timers.
 */
...
int zmk_widget_info_init(struct zmk_widget_info *widget, lv_obj_t *parent) {
    ...
    lv_obj_align(wpm_label, LV_ALIGN_TOP_RIGHT, 0, 0);
